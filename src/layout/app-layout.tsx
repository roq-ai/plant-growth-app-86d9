...
import { getAgricultures } from 'apiSdk/agricultures';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['plant-specialist'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AgricultureInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/agricultures` : null),
    () => getAgricultures({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Agriculture',
      path: isTenantUser ? `/agricultures/view/${data?.[0]?.id}` : '/agricultures',
      entity: 'agriculture',
